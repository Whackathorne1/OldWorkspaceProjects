...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        boolean isSubset = false;
            for (int i = 0; i < strSet.size(); i++) {
                isSubset = false;
                String element = strSetCopy.removeAny();
                int eLength = element.length();
                if (strLength < eLength) {
                    for (int j = 0; j < eLength - strLength || !isSubset; j++) {
                        String elementSubStr = element.substring(j, strLength - 1 + j);
                        if (elementSubStr.equals(str)) {
                            isSubset = true;
                        }
                    }
                }
                if (isSubset) {
                    strSetCopy.remove(element);
                }
            }
            if (isSubset) {
                strSetCopy.add(str);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            ...
            addToSetAvoidingSubstrings(result, s);
        }
        return result;
    }
     ...
     * result, if re-assembly is complete, might be the original text; but this
     ...
     * fragments, appropriately labeled.
     *
     * @param args
