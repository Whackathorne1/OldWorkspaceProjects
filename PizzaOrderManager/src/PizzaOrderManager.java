...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!file.atEOS()) {
            l = file.nextLine();
            int x = l.indexOf(',');
            String menuItem = l.substring(0, x);
            String stringPrice = l.substring(x + 1, l.length());
            int price = Integer.parseInt(stringPrice);
            priceMap.add(menuItem, price);
        }
        file.close();
    }
     ...
     *  by zero or more topping's (something defined in toppingPriceMap)
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        while (l.length() != 0) {
            if (i == 0 && !l.equals("") && sizePriceMap.hasKey(l)) {
                price = price + sizePriceMap.value(l);

            } else if (!l.equals("") && toppingPriceMap.hasKey(l)) {
                price = price + toppingPriceMap.value(l);
            }
            l = input.nextLine();
            i++;
        }
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        int oneHundred = 100;
        int cents = price % oneHundred;
        int dollars = (price / oneHundred);
    ...
    public static void main(String[] args) {
        ...
        int orderNumber = 1;
        /*
         * Get menus of sizes with prices and topping's with prices
         */
