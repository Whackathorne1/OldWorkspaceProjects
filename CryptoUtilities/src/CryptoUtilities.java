...
public final class CryptoUtilities {
    ...
    public static NaturalNumber randomNumber(NaturalNumber n) {
        ...
        if (n.isZero()) {
             ...
             * (integer) uniformly distributed in [0, 9] (i.e., a random digit)
             */
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.isZero()) {
            ...
            if (isEven(p)) {
                ...
                n.power(2); //n = (n^(p/2))^2
                x = n.divide(m);
                n.copyFrom(x); //n = n mod m
                p.multiply(two); //restoring p
            } else {
                //p is odd
                x = p.divide(two); //n^p = n^7, p = 3
                powerMod(n, p, m);
                n.power(2);
                n.multiply(copyN); // n = n * (n^(p/2))^2
                x = n.divide(m);
                n.copyFrom(x); //n = n mod m
                p.multiply(two); //p = 6
                p.increment(); //restore p for odd
            }
        }
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            nMinus1.decrement();
            //iWTC requires: n > 2 and 1 < w < n - 1
            for (int i = 0; i < candidates && isPrime; i++) {
                NaturalNumber w = randomNumber(n);
                if (w.compareTo(one) > 0 && w.compareTo(nMinus1) < 0) {
    ...
    //bare minimum of 3 test cases per method
    //check parameter modes (restores, updates, etc)
    //each method has a boundary/routine/challenging at the very least
    //best case scenario: have test cases for every situation possible
